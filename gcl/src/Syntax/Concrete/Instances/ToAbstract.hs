...
instance (Ord r, Hack.IsRange r) => ToAbstract (Program r) (A.Program (Maybe r)) r where
  toAbstract prog@(Program ds stmts') = do
    ...
    return $ A.Program defns decls globProps (pre ++ stmts) (Hack.maybeInfo prog)
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Definition r) [A.Definition (Maybe r)] r where
  toAbstract (TypeDefn tok name binders _ cons) = do
    (: [])
      <$> (A.TypeDefn (Hack.aToMaybeA name) (map Hack.aToMaybeA binders) <$> toAbstract cons <*> pure (Hack.rangeToA (rangeOf tok) Hack.<--> Hack.maybeInfo (Hack.info cons)))
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Declaration r) (A.Declaration (Maybe r)) r where
  toAbstract d = case d of
    ConstDecl _ decl -> do
      ...
      return $ A.ConstDecl name body prop (Hack.maybeInfo d)
    VarDecl _ decl -> do
      (name, body, prop) <- toAbstract decl
      return $ A.VarDecl name body prop (Hack.maybeInfo d)
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Stmt r) (A.Stmt (Maybe r)) r where
  toAbstract stmt = case stmt of
    Skip _ -> pure (A.Skip (Hack.maybeInfo stmt))
    Abort _ -> pure (A.Abort (Hack.maybeInfo stmt))
    Assign a _ b -> do
      A.Assign <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo stmt)
    AAssign x _ i _ _ e ->
      A.AAssign (A.Var (Hack.aToMaybeA x) (Hack.maybeInfo x))
        <$> toAbstract i
        <*> toAbstract e
        <*> pure
          (Hack.maybeInfo stmt)
    Assert _ a _ -> A.Assert <$> toAbstract a <*> pure (Hack.maybeInfo stmt)
    LoopInvariant _ a _ _ _ b _ ->
      A.LoopInvariant <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo stmt)
    Do _ a _ -> A.Do <$> toAbstract a <*> pure (Hack.maybeInfo stmt)
    If _ a _ -> A.If <$> toAbstract a <*> pure (Hack.maybeInfo stmt)
    ...
    Alloc p _ _ _ es _ -> A.Alloc (Hack.aToMaybeA p) <$> toAbstract es <*> pure (Hack.maybeInfo stmt)
    HLookup x _ _ e -> A.HLookup (Hack.aToMaybeA x) <$> toAbstract e <*> pure (Hack.maybeInfo stmt)
    HMutate _ e1 _ e2 ->
      A.HMutate <$> toAbstract e1 <*> toAbstract e2 <*> pure (Hack.maybeInfo stmt)
    Dispose _ e -> A.Dispose <$> toAbstract e <*> pure (Hack.maybeInfo stmt)
    Block _ p _ -> A.Block <$> toAbstract p <*> pure (Hack.maybeInfo stmt)

instance (Ord r, Hack.IsRange r) => ToAbstract (GdCmd r) (A.GdCmd (Maybe r)) r where
  toAbstract (GdCmd a _ b) =
    A.GdCmd <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo a Hack.<--> foldr (\x acc -> acc Hack.<--> Hack.maybeInfo x) Nothing b)
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Interval r) (A.Interval (Maybe r)) r where
  toAbstract i@(Interval a _ b) =
    A.Interval <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo i)
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Type r) (A.Type (Maybe r)) r where
  toAbstract t = case t of
    (TBase a) -> A.TBase <$> toAbstract a <*> pure (Hack.maybeInfo t)
    (TArray _ a _ b) ->
      A.TArray <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo t)
    (TOp op) -> pure $ A.TOp (Hack.aToMaybeA op)
    (TData n _) -> pure $ A.TData (Hack.aToMaybeA n) (Hack.maybeInfo t)
    (TApp l r) -> A.TApp <$> toAbstract l <*> toAbstract r <*> pure (Hack.maybeInfo l Hack.<--> Hack.maybeInfo r)
    (TMetaVar a _) -> pure $ A.TMetaVar (Hack.aToMaybeA a) (Hack.maybeInfo t)
    (TParen _ a _) -> do
      t' <- toAbstract a
      case t' of
        A.TBase a' _ -> pure $ A.TBase a' (Hack.maybeInfo t)
        A.TArray a' b' _ -> pure $ A.TArray a' b' (Hack.maybeInfo t)
        A.TTuple as' -> pure $ A.TTuple as'
        A.TFunc a' b' _ -> pure $ A.TFunc a' b' (Hack.maybeInfo t)
        A.TOp op -> pure $ A.TOp op
        A.TData name _ -> pure $ A.TData name (Hack.maybeInfo t)
        A.TApp a' b' _ -> pure $ A.TApp a' b' (Hack.maybeInfo t)
        A.TVar a' _ -> pure $ A.TVar a' (Hack.maybeInfo t)
        A.TMetaVar a' _ -> pure $ A.TMetaVar a' (Hack.maybeInfo t)
...
instance (Ord r, Hack.IsRange r) => ToAbstract (Expr r) (A.Expr (Maybe r)) r where
  toAbstract x = case x of
    ...
    Lit a -> A.Lit <$> toAbstract a <*> pure (Hack.maybeInfo x)
    Var a -> pure $ A.Var (Hack.aToMaybeA a) (Hack.maybeInfo x)
    Const a -> pure $ A.Const (Hack.aToMaybeA a) (Hack.maybeInfo x)
    Op a -> pure $ A.Op (Hack.aToMaybeA a)
    Chain ch -> A.Chain <$> toAbstract ch
    Arr arr _ i _ ->
      A.ArrIdx <$> toAbstract arr <*> toAbstract i <*> pure (Hack.maybeInfo x)
    App a b -> A.App <$> toAbstract a <*> toAbstract b <*> pure (Hack.maybeInfo x)
    Quant _ a b _ c _ d _ ->
      A.Quant
        ...
        <*> toAbstract c
        <*> toAbstract d
        <*> pure (Hack.maybeInfo x)
      where
        toAbstractQOp (QuantOp' qop) = case qop of
          Left op -> return (A.Op op)
          Right n@(Name _ l) -> return $ A.Const n l
    Case _ expr _ cases ->
      A.Case <$> toAbstract expr <*> toAbstract cases <*> pure (Hack.maybeInfo x)

instance (Ord r, Hack.IsRange r) => ToAbstract (Chain r) (A.Chain (Maybe r)) r where
  toAbstract chain = case chain of
    Pure expr -> A.Pure <$> toAbstract expr <*> pure (Hack.maybeInfo expr)
    More ch' op expr -> A.More <$> toAbstract ch' <*> pure (Hack.aToMaybeA op) <*> toAbstract expr <*> pure (Hack.maybeInfo expr)
