...
import GCL.Common
import GCL.Range (Range(..), MaybeRanged (maybeRangeOf), rangeOf, (<--->))
import Pretty.Util
...
tdefToAbs (TypeDefn tok name binders _ cons) = do
  ...
  return (A.TypeDefn name binders cons'
...
desugarClauses range clauses = do
  ...
  where arity = maximum (map (length . fst) clauses)
        mkCase :: [Name] -> A.Expr
        mkCase fnames =
...
instance ToAbstract Type A.Type where
  toAbstract t = case t of
    ...
    (TArray _ a _ b) -> A.TArray <$> toAbstract a <*> toAbstract b <*>
                                 pure (maybeRangeOf t)
    (TOp op) -> return $ A.TOp op
    (TData n _) -> return $ A.TData n (maybeRangeOf t)
    (TApp l r) -> A.TApp <$> toAbstract l <*> toAbstract r <*>
...
instance ToAbstract Expr A.Expr where
  toAbstract x = case x of
    ...
    Quant _ a b _ c _ d _ ->
      A.Quant (toAbstractQOp a) b <$> toAbstract c <*>
              toAbstract d <*> pure (maybeRangeOf x)
      where
        toAbstractQOp qop = case qop of
          Left op -> A.Op op
          Right n@(Name _ l) -> A.Const n l
    Case _ expr _ cases -> A.Case <$> toAbstract expr <*> toAbstract cases <*>
                           pure (maybeRangeOf x)
...
instance ToAbstract Chain A.Chain where
  toAbstract chain = case chain of
    ...
    More ch' op expr -> A.More <$> toAbstract ch' <*> pure op
                               <*> toAbstract expr <*> pure (maybeRangeOf expr)
