...
instance ToAbstract Stmt A.Stmt where
  toAbstract stmt = case stmt of
    ...
    AAssign x _ i _ _ e ->
      A.AAssign (A.Var x (maybeRangeOf x)) (toAbstract i) (toAbstract e) (maybeRangeOf stmt)
    ...
    HMutate _ e1 _ e2 -> A.HMutate (toAbstract e1) (toAbstract e2) (maybeRangeOf stmt)
...
instance ToAbstract Type A.Type where
  toAbstract t = case t of
    ...
    (TArray _ a _ b) -> A.TArray (toAbstract a) (toAbstract b) (maybeRangeOf t)
    (TOp op) -> A.TOp op
...
instance ToAbstract Expr A.Expr where
  toAbstract x = case x of
    ...
    Arr arr _ i _ -> A.ArrIdx (toAbstract arr) (toAbstract i) (maybeRangeOf x)
    App a b -> A.App (toAbstract a) (toAbstract b) (maybeRangeOf x)
    Quant _ a b _ c _ d _ ->
      A.Quant (toAbstractQOp a) b (toAbstract c) (toAbstract d) (maybeRangeOf x)
      where
        toAbstractQOp qop = case qop of
          Left op -> A.Op op
          Right n@(Name _ l) -> A.Const n l
    Case _ expr _ cases -> A.Case (toAbstract expr) (toAbstract cases) (maybeRangeOf x)
...
instance ToAbstract CaseClause A.CaseClause where
  toAbstract (CaseClause patt _ body) = A.CaseClause (toAbstract patt) (toAbstract body)
