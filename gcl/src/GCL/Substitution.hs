...
getCapturableNamesAndShrinkMapping mapping body =
  let -- collect all free variables in "body"
   ...
   in (freeVarsInMappedExprs, shrinkedMapping)
...
shrinkMapping expr mapping =
  let -- collect all free variables in the expression
      fv = Set.map nameToText (freeVars expr)
   in -- restrict the mapping with only free variables
      Map.restrictKeys mapping fv
