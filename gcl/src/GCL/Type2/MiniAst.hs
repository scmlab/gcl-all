...
instance Transform A.Type (Type Loc) where
...
instance Transform A.Expr (Expr Loc) where
  toMiniAst (A.Lit lit loc) = ELit loc lit
  toMiniAst (A.Var name loc) = EVar loc name
  toMiniAst (A.Op op) = EOp (locOf op) op
  toMiniAst (A.Chain chain) = EChain (locOf chain) (toMiniAst chain)
  toMiniAst (A.App e1 e2 loc) = EApp loc (toMiniAst e1) (toMiniAst e2)
  toMiniAst (A.Lam name expr loc) = ELam loc name (toMiniAst expr)
  toMiniAst (A.Tuple exprs) = ETuple NoLoc (NE.map toMiniAst $ NE.fromList exprs)
...
instance Transform A.Chain (Chain Loc) where
  ...
  fromMiniAst (Pure loc e1 op e2) = A.More (A.Pure e1' (locOf e1')) op (fromMiniAst e2) loc
    where
      e1' = fromMiniAst e1
