...
definition =
  ...
  where
    run = parserIso Parser.definition
...
definitionBlock =
  ...
  where
    run = parserIso Parser.definitionBlock
...
declaration =
  ...
  where
    run = parserIso Parser.declaration
...
statement =
  ...
  where
    run = parserIso Parser.statement
...
parseError =
  testGroup
    ...
    [ testCase "variable keyword collision" $
        runDeclaration
          "var if : Int"
          "Parse Error 1:5-7 unexpected 'if'\nexpecting identifier\n"
